	...
	<form action="/expense/MyExpenseServlet" id="f1" method="post" class="form-inline">
		<div class="row alert alert-info" style="margin: 0px; padding: 5px;">
			...
			<select class="form-control" name="expenseState">
				...
				<option value="2" ${expense.expenseState=='3'?'selected':'' }>经理审核通过</option>
				<option value="3" ${expense.expenseState=='4'?'selected':'' }>财务审核通过</option>
				<option value="-1" ${ea.expenseState=='-3'?'selected':''}>经理审核未通过</option>
				<option value="-2" ${ea.expenseState=='-4'?'selected':''}>财务审核未通过</option>
			</select>
			</div>
		...
		<div class="row" style="padding: 15px;">
			<d:table class="table table-hover table-condensed" name="list" pagesize="10" requestURI="expense/MyExpenseServlet">
				...
				<d:column property="updateUrl" title="修改报销单"></d:column>
